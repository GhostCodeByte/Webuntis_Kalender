...
Eine Expo/React-Native-App, die deinen persönlichen WebUntis-Stundenplan filtert, anzeigt und automatisch mit dem Google Kalender synchronisiert.
...
- **Google Kalender Sync**: OAuth-basierte Verbindung, eindeutige Event-IDs, wählbarer Zielkalender, Sync-Horizont in Tagen konfigurierbar.
...
3. **Google Cloud Projekt** inkl. OAuth Client (Typ "Installed" oder "Web") mit Kalender-Scope `https://www.googleapis.com/auth/calendar` und Redirect `webuntisKal://oauthredirect`.
...
### Anzeige
- **Tage in die Zukunft**: legt fest, wie weit Ereignisse in Google eingetragen werden.
...
- `src/services/calendarService.ts`: Formt Daten zu Google-kompatiblen Events.
- `src/services/syncService.ts`: Orchestriert WebUntis Fetch → Transformation → Google Sync.
...
- `app/settings.tsx`: Alle Konfigurationen + Google OAuth Button.
